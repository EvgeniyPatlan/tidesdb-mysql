...
/* tidesdb_opts_for_table forward-decl and the TDB_*_OPTIONS macros
   moved to ha_tidesdb.h during the A-7 inplace-alter extraction so
   tidesdb_inplace_alter.cc sees them too. The struct forward-decls
   (ha_table_option_struct / ha_index_option_struct /
   ha_field_option_struct) also live there now.

   tdb_sanitize_for_log is likewise declared in ha_tidesdb.h so the
   FTS TU can reach it; defined alongside the path validation helpers
   later in this file. */
...
/* Spatial-index subsystem moved to tidesdb_spatial.{h,cc} during the
   A-2 architectural extraction. ha_tidesdb.cc keeps only the call
   sites (index_read for HA_READ_MBR_*, write_row / update_row /
   delete_row spatial branches, inplace ALTER spatial fast paths).
   Symbols still visible at module scope via tidesdb_spatial.h:
   is_spatial_index, spatial_compute_mbr, spatial_build_key,
   spatial_build_value, spatial_parse_query_mbr, spatial_mbr_predicate,
   spatial_decompose_ranges, hilbert_xy2d_64, double_to_lex_uint32,
   encode_hilbert_be, decode_hilbert_be, tdb_mbr_t,
   SPATIAL_HILBERT_KEY_LEN, SPATIAL_MBR_VALUE_LEN. */
...
/* ha_table_option_struct moved to ha_tidesdb.h (A-7 pass) so the
   inplace-alter TU can construct it. */
...
/* ha_field_option_struct / ha_index_option_struct moved to
   ha_tidesdb.h (A-7 pass). (deleted) #if 0 *_option_list MariaDB-only
   dead code; see git history if needed. */
...
/* tdb_isolation_map moved to ha_tidesdb.h (A-7 pass) as an inline
   constexpr array so build_cf_config and the inplace-alter TU share
   one definition. */
...
/* tdb_empty_val moved to ha_tidesdb.h (A-7 pass) as inline constexpr. */
...
*/
int schema_cf_store_frm(const char *path, const uchar *frm_data, size_t frm_len)
{
...
/* The inplace-ALTER state machine (check_if_supported_inplace_alter,
   prepare/inplace/commit_inplace_alter_table, check_if_incompatible_data)
   moved to tidesdb_inplace_alter.cc during the A-7 architectural
   extraction. They remain ha_tidesdb member functions -- only the
