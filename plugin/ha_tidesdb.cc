...
static void tdb_fts_blend_chars_update(MYSQL_THD thd, SYS_VAR *var, void *var_ptr,
                                       const void *save)
{
...
static bool tdb_load_stopwords_from_table_spec(const char *table_spec)
{
    ...
    while (tidesdb_iter_valid(iter))
    {
        ...
        if (tidesdb_iter_value(iter, &val, &val_size) == TDB_SUCCESS && val && val_size > 0)
        {
            ...
            /* We skip row header (magic + null_bytes_stored + field_count) */
            if (data_len > ROW_HEADER_SIZE)
            {
                data += ROW_HEADER_SIZE;
                data_len -= ROW_HEADER_SIZE;
                /* We skip null bitmap (NULL_BITMAP_SINGLE_FIELD byte for up to 8 fields) */
...
static void tidesdb_backup_dir_update(THD *thd, SYS_VAR *, void *var_ptr,
                                      const void *save)
{
...
static void tidesdb_checkpoint_dir_update(THD *thd, SYS_VAR *, void *var_ptr,
                                          const void *save)
{
    const char *new_dir = *static_cast<const char *const *>(save);
