...
/* MUST be the first include — provides MariaDB->MySQL compat shims that need
 * to be defined before any MySQL header. See storage/tidesdb/tidesdb_compat.h. */
#include "storage/tidesdb/tidesdb_compat.h"
...
#include "sql/table.h"   /* TABLE, TABLE_SHARE, dbug_tmp_use_all_columns */
#include "sql/field.h"   /* Field, Field_blob, Field_varstring */
#include "my_base.h"
#include "thr_lock.h"
...
class ha_tidesdb : public handler
{
   ...
   public:
    ...
    /* Pure virtual in MySQL's handler — engine display name. */
    const char *table_type() const override { return "TIDESDB"; }
    ...
    const char *index_type(uint key_number);  /* override removed: signature drift in MySQL */
    ...
    /* Base class returns 255 — too small for any real-world VARCHAR index
     * (e.g. VARCHAR(64) with utf8mb4 = 256 bytes per part). Match InnoDB's
     * default of 3072 bytes. */
    uint max_supported_key_part_length(HA_CREATE_INFO *) const override
    {
        return MAX_KEY_LENGTH;
    }

    IO_AND_CPU_COST keyread_time(uint index, ulong ranges, ha_rows rows, ulonglong blocks)
        /* override removed: MariaDB-only cost virtual (MySQL uses double read_time/scan_time) */
    {
    ...
    IO_AND_CPU_COST rnd_pos_time(ha_rows rows) /* override removed: MariaDB-only */
    {
    ...
    /* DML */
    int write_row(uchar *buf) override;                        /* MariaDB had `const uchar*` */
    ...
    void ft_end();  /* override removed: MariaDB-only */
    FT_INFO *ft_init_ext(uint flags, uint inx, String *key) override;
    int ft_read(uchar *buf) override;

    /* Bulk insert hint (LOAD DATA, multi-row INSERT) */
    void start_bulk_insert(ha_rows rows) override;  /* MariaDB had a flags arg */
    ...
    bool start_bulk_update();   /* override removed: MariaDB-only */
    int tdb_end_bulk_update();  /* renamed from end_bulk_update to avoid conflict with MySQL's void virtual */
    /* MySQL's bulk_update_row is `(const uchar*, uchar*, uint*)` and the base
     * impl asserts(false); without a real override here, single-row UPDATE
     * crashes. Match MySQL's signature so the override actually takes. */
    int bulk_update_row(const uchar *old_data, uchar *new_data,
                        uint *dup_key_found) override;
    bool start_bulk_delete();   /* override removed: MariaDB-only */
    int end_bulk_delete();      /* override removed: MariaDB-only */
    ...
    ha_rows multi_range_read_info_const(uint keyno, RANGE_SEQ_IF *seq, void *seq_init_param,
                                        ...
                                        Cost_estimate *cost);  /* override removed: MariaDB-only sig */
    ...
    int reset_auto_increment(ulonglong value);  /* override removed: MySQL has it
                                                   but with different signature; engine-internal for now */
    ...
    /* MariaDB had a 4th `page_range *pages` arg — dropped in MySQL */
   ...
   public:
   protected:
    double scan_time() override;  /* MariaDB returns IO_AND_CPU_COST; converted to double */
